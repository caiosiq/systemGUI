...
class TC720():
    ...
    def __init__(self, address, name='TC-720', mode=0, control_type=0,
        ...
        self.ser = serial.Serial(self.address, timeout=2, baudrate=230400, stopbits=serial.STOPBITS_ONE,
        ...
        self.verboseprint(
            'Mode set to: {}, control type set to: {}, temperature set to: {}C'.format(self.mode, self.control_type,
                                                                                       self.default_temp))
    ...
    def int_to_hex(self, integer):
        ...
        if abs(integer) > 32768:
            raise ValueError(
                'Can not encode positive or negative integers larger than  32768 in length 4 hexadecimal number.')
    ...
    def message_builder(self, command, value='0000'):
        ...
        if type(command) != str:
            try:
                ...
                raise ValueError(
                    'Invalid command input: "{}", Type:"{}". Input should be a string of length 2'.format(command, type(
                        command)))
        if len(command) != 2:
            raise ValueError(
                'Invalid command input: "{}", Type:"{}". Input should be a string of length 2'.format(command,
                                                                                                      type(command)))
        ...
        if type(value) != str:
            try:
                ...
                raise ValueError(
                    'Invalid message input: "{}", Type:"{}". Input should be a string of length 4'.format(value,
                                                                                                          type(value)))
        if len(value) != 4:
            raise ValueError(
                'Invalid message input: "{}", Type:"{}". Input should be a string of length 4'.format(value,
        ...
        # Send write commands
            ...
            for n in range(5):
                ...
                # Unknown error
                    ...
                    checksum_error = ''
                    self.verboseprint(
                        '    {} Error: Temperature controller did not correctly receive the command.'.format(self.name))
                time.sleep(0.05)
            else:
                raise Exception(
                    'Could not correctly send "{}" to temperature controller: {}. {}'.format(''.join(message[:-1]),
                                                                                             self.name, checksum_error))
        ...
        try:
            ...
            while True:
                ...
                # Timeout check
                    ...
                    warnings.warn(
                        'Did not receive a response from temperature control unit "{}" within timout period.'.format(
                            self.name))
    ...
    def validate_data(self, input):
        ...
        if type(input) != int or not 1 <= input <= 8:
            raise ValueError(
                'Invalid location: "{}", type: "{}. Must be an integer in the range 1-8.'.format(input, type(input)))
    ...
    def check_mode(self, desired_mode):
        ...
        if not cur_mode == desired_mode:
            warnings.warn(
                'TC720: {} is not set in the right mode to use this function. Current mode: {}, set the machine in the {} mode using set_mode({})'.format(
                    self.name, cur_mode, desired_mode, desired_mode))
            return False
        else:
            return True
    ...
    def set_soak_temp(self, location, temperature):
        ...
        if type(location) != int or (1 < location > 8):
            raise ValueError('Invalid location: "{}", type: "{}. Must be a integer in the range 1-8.'.format(location,
                                                                                                             type(
                                                                                                                 location)))
        # Check mode
        self.check_mode(1)
    ...
    def set_soak_time(self, location, time):
        ...
        if type(time) != int or (1 < time > 32768):  # half 2**16
            raise ValueError('Invalid time: "{}", type: "{}. Must be a integer in the range 1-32768.'.format(location,
                                                                                                             type(
                                                                                                                 location)))
        # Check mode
        self.check_mode(1)
    ...
    def get_sequence(self, location='all'):
        ...
        for i in location:
            seq = np.append(seq, [
                [i, self.get_soak_temp(i), self.get_ramp_time(i), self.get_soak_time(i), self.get_repeats(i),
                 self.get_repeat_location(i)]], axis=0)
        ...
        while True:
            ...
            if counter >= (timeout * 60):  # Raise and exception after the timeout period
                ...
                if set_idle == True:
                    self.set_idle()
                    raise Exception(
                        'Temperature could not be reached in {} minutes, check {} system.'.format(timeout, self.name))
                else:
                    warnings.warn(
                        'Temperature could not be reached in {} minutes, check {} system.'.format(timeout, self.name))
                    break
